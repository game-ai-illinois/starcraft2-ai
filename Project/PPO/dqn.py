...
def eps_decay(obs, sess, DQN_sc2, eps_s, eps_f,decay_rate, timestep, timestep_obs):
    action =actions.FUNCTIONS.no_op()
    arg=None #default value for debugging
    
    if eps_s - decay_rate*timestep < eps_f:
        epsilon= eps_f
    else:
        epsilon =eps_s - decay_rate*timestep
...
#initialization of weights and biases
        ...
        self.Yhat = tf.nn.relu( tf.matmul(self.Z3, self.W4) + self.b4 )
...
def train(env, sess, agent,exp, saver, total_episodes,dqn_sc2, eps_s, eps_f,decay_rate,time,batch_size , gamma  ):
    ...
    for episode in range(total_episodes):
        ...
        while True: 
            state_old = state
            [step_actions , step_actions_array] = eps_decay(state.get().reshape((1,*(state.get().shape) )),sess,dqn_sc2, eps_s, eps_f,decay_rate,time ,timesteps[0] )
...
def test(env,  agent, saver,dqn_sc2, exp):
    print("testing!")
...
def train_iteration(env, sess, agent,exp, saver, total_episodes, dqn_sc2, eps_s, eps_f,decay_rate,time,batch_size , gamma ):
    time=train(env, sess, agent,exp, saver, total_episodes, dqn_sc2, eps_s, eps_f,decay_rate,time,batch_size , gamma )
