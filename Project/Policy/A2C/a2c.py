...
def get_reward( timesteps, done) :
    ...
    living_cost =10
    ally_coef = 0.1
    # print(len(zergling) +len(baneling))
    if len(zergling) +len(baneling) ==0:
            print("Maybe we won?")
    if not done:
...
def action(non_spatial_policy, primary_spatial_policy, secondary_spatial_policy,tertiary_spatial_policy, obs) :
    ...
    # print(non_spatial_policy.shape, primary_spatial_policy.shape, secondary_spatial_policy.shape, (is_primary,  is_spatial_action))
    ...
    if action_non_spatial == 0: #do nothing
        result = [actions.FUNCTIONS.no_op() ,\
         one_hot(action_non_spatial,non_spatial_policy_size) , np.zeros((screen_y_len,screen_x_len)) , np.zeros((screen_y_len,screen_x_len)), np.zeros((screen_y_len,screen_x_len)) ,(False, False)] 
    elif action_non_spatial == 1: #select box
        x_secondary,y_secondary =random_2d_choice(secondary_spatial_policy)
        x_tertiary, y_tertiary =random_2d_choice(tertiary_spatial_policy)
        ...
        tertiary_determine = np.zeros((screen_y_len,screen_x_len))
        tertiary_determine[y_tertiary,x_tertiary] =1.0 # row number is y, column number is x

        result = [actions.FUNCTIONS.select_rect("select", (x_secondary, y_secondary) , (x_tertiary, y_tertiary) ), \
         one_hot(action_non_spatial,non_spatial_policy_size) , np.zeros((screen_y_len,screen_x_len)), secondary_determine , tertiary_determine, (True, True)]
        ...
        if can_do(obs, actions.FUNCTIONS.Attack_screen.id):
            result = [actions.FUNCTIONS.Attack_screen("queued", (x,y )) , \
                 one_hot(action_non_spatial,non_spatial_policy_size) ,  primary_determine , np.zeros((screen_y_len,screen_x_len)), np.zeros((screen_y_len,screen_x_len)), (True, False)]
        ...
        if can_do(obs, actions.FUNCTIONS.Move_screen.id):
            result = [actions.FUNCTIONS.Move_screen("queued", (x,y )) ,\
                 one_hot(action_non_spatial,non_spatial_policy_size) ,  primary_determine , np.zeros((screen_y_len,screen_x_len)), np.zeros((screen_y_len,screen_x_len)), (True, False)]
        else: #if  not available, do nothing
            result = [actions.FUNCTIONS.no_op() , \
                one_hot(action_non_spatial,non_spatial_policy_size)  , primary_determine , np.zeros((screen_y_len,screen_x_len)), np.zeros((screen_y_len,screen_x_len)) , (True, False)] 
        ...
        if can_do(obs, actions.FUNCTIONS.Patrol_screen.id):
            result = [actions.FUNCTIONS.Patrol_screen("queued", (x,y )) , \
                one_hot(action_non_spatial,non_spatial_policy_size) ,  primary_determine , np.zeros((screen_y_len,screen_x_len)), np.zeros((screen_y_len,screen_x_len)), (True, False)]
        else: #if  not available, do nothing
            result = [actions.FUNCTIONS.no_op() , \
                one_hot(action_non_spatial,non_spatial_policy_size)  , primary_determine , np.zeros((screen_y_len,screen_x_len)), np.zeros((screen_y_len,screen_x_len)) , (True, False)] 
        ...
        if can_do(obs, actions.FUNCTIONS.HoldPosition_quick.id):
            result = [actions.FUNCTIONS.HoldPosition_quick("queued") , \
                one_hot(action_non_spatial,non_spatial_policy_size) , np.zeros((screen_y_len,screen_x_len)), np.zeros((screen_y_len,screen_x_len)) , np.zeros((screen_y_len,screen_x_len)), (False, False)]
        else: #if  not available, do nothing
            result = [actions.FUNCTIONS.no_op() ,\
                 one_hot(action_non_spatial,non_spatial_policy_size)  , np.zeros((screen_y_len,screen_x_len)), np.zeros((screen_y_len,screen_x_len)) , np.zeros((screen_y_len,screen_x_len)) , (False, False)] 
        ...
        if can_do(obs, actions.FUNCTIONS.Stop_quick.id):
            result = [actions.FUNCTIONS.Stop_quick("queued") , \
                one_hot(action_non_spatial,non_spatial_policy_size) , np.zeros((screen_y_len,screen_x_len)),  np.zeros((screen_y_len,screen_x_len)) , np.zeros((screen_y_len,screen_x_len)), (False, False)]
        else: #if  not available, do nothing
            result = [actions.FUNCTIONS.no_op() ,\
                 one_hot(action_non_spatial,non_spatial_policy_size)  , np.zeros((screen_y_len,screen_x_len)), np.zeros((screen_y_len,screen_x_len)) , np.zeros((screen_y_len,screen_x_len)) , (False, False)] 
...
class NN:
    def __init__(self, screen_size, minimap_size, non_spatial_size, non_spatial_action_size, spatial_action_size,  learning_rate, name):
        ...
        # secondary and tertiary action space is used for select_rect action
        with tf.variable_scope(self.name):
            ...
            self.secondary_actions_= tf.placeholder(tf.float32, shape=[None, *spatial_action_size], name='secondary_actions_')
            self.tertiary_actions_= tf.placeholder(tf.float32, shape=[None, *spatial_action_size], name='tertiary_actions_')
            ...
            self.is_spatial_action = tf.placeholder(tf.float32, [None,1], name="is_spatial_action") #indicates if secondary and tertiary spatial action space was used
            ...
            self.secondary_conv = tf.layers.conv2d(inputs = self.state_representation_,
                                    ...
                                    activation=None,
                                    name = "secondary_conv")
            self.secondary_action_conv_ = softmax(self.secondary_conv, name = "secondary_action_conv_")

            self.tertiary_conv = tf.layers.conv2d(inputs = self.state_representation_,
                                    filters = 1,
                                    kernel_size = [1,1],
                                    strides = [1,1],
                                    padding = "same",
                                    ...
                                    activation=None,
                                    name = "tertiary_conv")
            self.tertiary_action_conv_ = softmax(self.tertiary_conv, name = "tertiary_action_conv_")
            ...
            self.entropy_spatial_secondary = -tf.reduce_sum(self.secondary_action_conv_ * tf.log(C + self.secondary_action_conv_)) 
            self.entropy_spatial_tertiary = -tf.reduce_sum(self.tertiary_action_conv_ * tf.log(C + self.tertiary_action_conv_)) 
            ...
            self.secondary_action_conv_out_ = tf.reduce_sum(self.secondary_action_conv_, axis=-1)
            self.policy_loss_spatial_secondary = tf.reduce_sum(-tf.log(C+ tf.reduce_sum(self.secondary_action_conv_out_* self.secondary_actions_))*self.advantages_spatial_ ) 
            self.tertiary_action_conv_out_ = tf.reduce_sum(self.tertiary_action_conv_, axis=-1)
            self.policy_loss_spatial_tertiary = tf.reduce_sum(-tf.log(C+  tf.reduce_sum(self.tertiary_action_conv_out_*  self.tertiary_actions_))*self.advantages_spatial_ ) 
            ...
            self.loss_tertiary =  self.policy_loss_spatial_tertiary + self.entropy_spatial_tertiary*self.entropy_coef
            
            
            self.loss_total = self.loss_noop + self.is_primary * self.loss_primary + self.is_spatial_action* (self.loss_secondary + self.loss_tertiary)
...
def test( sess, agent, NN, env, screen_size, minimap_size, non_spatial_size, non_spatial_action_size,spatial_action_size, total_episodes):
    ...
    memory["action_spatial_tertiary"] = []
    memory["reward"] = []
    memory["value"] = []
    memory["is_primary"] = []
    memory["is_spatial_action"] = []
    for episode in range(total_episodes):
        ...
        action_spatial_tertiary_stack = np.empty((0  ,*spatial_action_size), float)
        reward_stack= np.empty((0  ,1), float)
        value_stack = np.empty((0  ,1), float)
        is_primary_stack = np.empty((0  ,1), float)
        is_spatial_action_stack = np.empty((0  ,1), float)
        ...
        while (not done) and timer < timer_limit:
            ...
            non_spatial_policy, p_spatial_policy, s_spatial_policy,  t_spatial_policy , value = sess.run([NN.policy, NN.primary_action_conv_,\
             NN.secondary_action_conv_, NN.tertiary_action_conv_, NN.value], feed_dict=feed_dict  )
            # print("spatial policy sum: ", np.sum(p_spatial_policy))
            # print("spatial policy : ", (p_spatial_policy))

            
            [action_step, non_spatial_determine , primary_spatial_determine , secondary_spatial_determine, tertiary_spatial_determine, is_tuple ] = action(\
                non_spatial_policy, p_spatial_policy, s_spatial_policy, t_spatial_policy, timesteps_current) #is_tuple = (is_primary, is_spatial_action)
            ...
            action_spatial_tertiary_stack = np.vstack((action_spatial_tertiary_stack, tertiary_spatial_determine.reshape(1 , *tertiary_spatial_determine.shape )))
            ...
            is_spatial_action_stack = np.vstack((is_spatial_action_stack,is_tuple[1]))
        ...
        memory["action_spatial_tertiary"].append(action_spatial_tertiary_stack)
        memory["reward"].append(reward_stack)
        memory["value"].append(value_stack)
        memory["is_primary"].append(is_primary_stack)
        memory["is_spatial_action"].append(is_spatial_action_stack)
...
screen_size, minimap_size, non_spatial_size, non_spatial_action_size,spatial_action_size): 
    ...
    tertiary_spatial_action_batch = np.empty((0,*spatial_action_size),float)
    reward_batch =   np.empty((0,1),float)
    current_value_batch =  np.empty((0,1),float)
    is_primary_batch =  np.empty((0,1),float)
    is_spatial_action_batch =  np.empty((0,1),float)
    ...
    for iteration in range(batch_size):
        ...
        tertiary_spatial_memory_clip = memory["action_spatial_tertiary"][i][j:j+batch_frame_len]
        tertiary_spatial_action_batch = np.vstack((tertiary_spatial_action_batch, tertiary_spatial_memory_clip ))
        ...
        is_spatial_action_memory_clip = memory["is_spatial_action"][i][j:j+batch_frame_len]
        is_spatial_action_batch =  np.vstack((is_spatial_action_batch, is_spatial_action_memory_clip)) 
    ...
    feed_dict={NN.screen_input_: screen_batch,
                                    ...
                                    NN.secondary_actions_ : secondary_spatial_action_batch,
                                    NN.tertiary_actions_ : tertiary_spatial_action_batch,
                                    NN.advantages_non_spatial_: advantage_non_spatial_batch,
                                    NN.rewards_: discount_reward_batch,
                                    NN.is_primary: is_primary_batch,
                                    NN.is_spatial_action : is_spatial_action_batch
                                     }
