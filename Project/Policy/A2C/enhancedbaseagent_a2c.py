...
def run_game(unused_argv ):
    ...
    total_episodes = 50   #prev 200    # Total episodes for training       
    iter_num = 1000000 #prev 20
    ...
    existing_model = False  #True if there's an existing model you would like to load
    model_path = "./models/a2cVTest.ckpt"
    try:
        with sc2_env.SC2Env(
            map_name=mapname,
            ...
            with tf.Session() as sess:
                if existing_model == True:
                    print ('Loading Model...')
                    saver.restore(sess, "./models/a2cVTest.ckpt")
                else:
