...
import random
from a2c_simple_stacked import *
import numpy as np
...
def run_game(unused_argv ):
    ...
    screen_size=(4*17,84,84) #four times 17,84,84 frames
    minimap_size=(4*7,84,84) #four times 7,84,84 frames
    non_spatial_size = (4*11)
    ...
    model_path = "./models/a2c_simple_stacked_now.ckpt"
    try:
        with sc2_env.SC2Env(
            map_name=mapname,
            ...
            # gpu_memory_allocation = tf.GPUOptions(per_process_gpu_memory_fraction=1.0)
            # config= tf.ConfigProto(gpu_options=gpu_memory_allocation)
