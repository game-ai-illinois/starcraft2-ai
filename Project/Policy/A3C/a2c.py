...
def action(non_spatial_policy, primary_spatial_policy, secondary_spatial_policy, obs) :
    ...
    '''
    #non_spatial_policy_list = ["do nothing", "select box","attack at", "move at", "patrol", "hold position", "stop"]
    # print(non_spatial_policy.shape, primary_spatial_policy.shape, secondary_spatial_policy.shape, (is_primary,  is_secondary))
    non_spatial_policy_size = 4
    ...
    if action_non_spatial == 0: #do nothing
        result = [actions.FUNCTIONS.no_op() , one_hot(action_non_spatial,non_spatial_policy_size) , np.zeros((screen_y_len,screen_x_len)) , np.zeros((screen_y_len,screen_x_len)) ,(False, False)] 
    elif action_non_spatial == 1: #select box
        x_primary,y_primary =random_2d_choice(primary_spatial_policy)
        x_secondary, y_secondary =random_2d_choice(secondary_spatial_policy)
        primary_determine = np.zeros((screen_y_len,screen_x_len))
        primary_determine[y_primary,x_primary] =1.0 # row number is y, column number is x
        secondary_determine = np.zeros((screen_y_len,screen_x_len))
        secondary_determine[y_secondary,x_secondary] =1.0 # row number is y, column number is x

        result = [actions.FUNCTIONS.select_rect("select", (x_primary,y_primary) , (x_secondary, y_secondary) ), \
         one_hot(action_non_spatial,non_spatial_policy_size) , primary_determine , secondary_determine, (True, True)]
         
    elif action_non_spatial == 2: #attack at
        ...
        if can_do(obs, actions.FUNCTIONS.Attack_screen.id):
            result = [actions.FUNCTIONS.Attack_screen("now", (x,y )) , one_hot(action_non_spatial,non_spatial_policy_size) ,  primary_determine , np.zeros((screen_y_len,screen_x_len)), (True, False)]
        else: #if  not available, do nothing
            result = [actions.FUNCTIONS.no_op() , one_hot(action_non_spatial,non_spatial_policy_size), primary_determine , np.zeros((screen_y_len,screen_x_len)) , (True, False)] 
    elif action_non_spatial == 3:    # move at
        x,y = random_2d_choice(primary_spatial_policy)
        primary_determine = np.zeros((screen_y_len,screen_x_len))
        primary_determine[y ,x ] =1.0 # row number is y, column number is x
        if can_do(obs, actions.FUNCTIONS.Move_screen.id):
            result = [actions.FUNCTIONS.Move_screen("now", (x,y )) , one_hot(action_non_spatial,non_spatial_policy_size) ,  primary_determine , np.zeros((screen_y_len,screen_x_len)), (True, False)]
        else: #if  not available, do nothing
            result = [actions.FUNCTIONS.no_op() , one_hot(action_non_spatial,non_spatial_policy_size)  , primary_determine , np.zeros((screen_y_len,screen_x_len)) , (True, False)] 
    elif action_non_spatial == 4:    # patrol
        x,y = random_2d_choice(primary_spatial_policy)
        primary_determine = np.zeros((screen_y_len,screen_x_len))
        primary_determine[y ,x ] =1.0 # row number is y, column number is x
        if can_do(obs, actions.FUNCTIONS.Patrol_screen.id):
            result = [actions.FUNCTIONS.Patrol_screen("now", (x,y )) , one_hot(action_non_spatial,non_spatial_policy_size) ,  primary_determine , np.zeros((screen_y_len,screen_x_len)), (True, False)]
        else: #if  not available, do nothing
            result = [actions.FUNCTIONS.no_op() , one_hot(action_non_spatial,non_spatial_policy_size)  , primary_determine , np.zeros((screen_y_len,screen_x_len)) , (True, False)] 
    elif action_non_spatial == 5:    # hold
        if can_do(obs, actions.FUNCTIONS.HoldPosition_quick.id):
            result = [actions.FUNCTIONS.HoldPosition_quick() , one_hot(action_non_spatial,non_spatial_policy_size) ,  primary_determine , np.zeros((screen_y_len,screen_x_len)), (False, False)]
        else: #if  not available, do nothing
            result = [actions.FUNCTIONS.no_op() , one_hot(action_non_spatial,non_spatial_policy_size)  , primary_determine , np.zeros((screen_y_len,screen_x_len)) , (False, False)] 
    else:    # stop
        if can_do(obs, actions.FUNCTIONS.Stop_quick.id):
            result = [actions.FUNCTIONS.Stop_quick() , one_hot(action_non_spatial,non_spatial_policy_size) ,  primary_determine , np.zeros((screen_y_len,screen_x_len)), (False, False)]
        else: #if  not available, do nothing
            result = [actions.FUNCTIONS.no_op() , one_hot(action_non_spatial,non_spatial_policy_size)  , primary_determine , np.zeros((screen_y_len,screen_x_len)) , (False, False)] 
...
class NN:
    def __init__(self, screen_size, minimap_size, non_spatial_size, non_spatial_action_size, spatial_action_size,  learning_rate, name):
        ...
        self.entropy_coef =0.001 # assuming entropy penalty means entropy coeff
        # self.value_coef = 0.5
        ...
        with tf.variable_scope(self.name):
            ...
            # self.primary_advantages_spatial_= tf.placeholder(tf.float32, shape=[None, *spatial_action_size], name='primary_advantages_spatial_')
            # self.secondary_advantages_spatial_= tf.placeholder(tf.float32, shape=[None, *spatial_action_size], name='secondary_advantages_spatial_')
            ...
            self.is_secondary = tf.placeholder(tf.float32, [None,1], name="is_secondary") #indicates if secondary spatial action space was used
            
            #convolution layer for feature screen
            # Screen Input is 17x84x84
...
###############################################################################################################################################################
            ...
            # LSTM
             ...
             #Inputs
            #self.batchX = tf.placeholder(tf.float32, [batch_size, backprop_length])
            #self.batchY = tf.placeholder(tf.int32, [batch_size, backprop_length])

            # Taking flattened conv layer 'self.flatten' as input into LSTM cell
            self.lstm_cell = tf.nn.rnn_cell.BasicLSTMCell(state_size, state_is_tuple=True)
            self.cell_state = tf.placeholder(tf.float32, [batch_size, state_size])
            self.hidden_state = tf.placeholder(tf.float32, [batch_size, state_size])
            self.init_state = tf.nn.rnn_cell.LSTMStateTuple(cell_state, hidden_state)
                                                
            self.W = tf.Variable(np.random.rand(state_size, num_classes), dtype=tf.float32)
            self.b = tf.Variable(np.random.rand(1, num_classes), dtype=tf.float32)

            self.inputs_series = tf.split(batchX, backprop_length, 1)
            # self.labels_series = tf.unstack(batchY, axis=1)

            self.cell = tf.nn.rnn_cell.BasicLSTMCell(state_size, state_is_tuple=True)
            self.states_series, current_state = tf.nn.static_rnn(cell, inputs_series, init_state)

            # self.logits_series = [tf.matmul(state, W) + b for state in states_series]
            # Assuming 
            self.predictions_series = [tf.nn.softmax(logits) for logits in logits_series]

            # self.losses = [tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits, labels=labels) for logits, labels in zip(logits_series,labels_series)]
            # self.total_loss = tf.reduce_mean(losses)

            # self.train_step = tf.train.AdagradOptimizer(0.3).minimize(total_loss)
...
###############################################################################################################################################################
            ...
            self.primary_conv = tf.layers.conv2d(inputs = self.state_representation_,
                                    ...
                                    activation=None,
                                    name = "primary_conv")
            self.primary_action_spatial_ =softmax(self.primary_conv, name = "primary_action_spatial_")
            
            self.secondary_conv = tf.layers.conv2d(inputs = self.state_representation_,
                                    filters = 1,
                                    kernel_size = [1,1],
                                    strides = [1,1],
                                    padding = "same",
                                    ...
                                    activation=None,
                                    name = "secondary_conv")
            self.secondary_action_spatial_ = softmax(self.secondary_conv, name = "secondary_action_spatial_")
            ...
            self.entropy_spatial_primary = -tf.reduce_sum(self.primary_action_spatial_ * tf.log(C + self.primary_action_spatial_)) 
            self.entropy_spatial_secondary = -tf.reduce_sum(self.secondary_action_spatial_ * tf.log(C + self.secondary_action_spatial_)) 
            ...
            self.policy_loss_spatial_primary = tf.reduce_sum(-tf.log(C+ self.primary_action_spatial_)*self.advantages_spatial_) 
            self.policy_loss_spatial_secondary = tf.reduce_sum(-tf.log(C+ self.secondary_action_spatial_)*self.advantages_spatial_ ) 
            ...
            self.loss_secondary =  self.policy_loss_spatial_secondary + self.entropy_spatial_secondary*self.entropy_coef
            
            self.loss_total = self.loss_noop + self.is_primary * self.loss_primary + self.is_secondary* self.loss_secondary
...
def test( sess, agent, NN, env, screen_size, minimap_size, non_spatial_size, non_spatial_action_size,spatial_action_size, total_episodes):
    ...
    memory["action_spatial_secondary"] = []
    memory["reward"] = []
    memory["value"] = []
    memory["is_primary"] = []
    memory["is_secondary"] = []
    for episode in range(total_episodes):
        ...
        action_spatial_secondary_stack = np.empty((0  ,*spatial_action_size), float)
        reward_stack= np.empty((0  ,1), float)
        value_stack = np.empty((0  ,1), float)
        is_primary_stack = np.empty((0  ,1), float)
        is_secondary_stack = np.empty((0  ,1), float)
        
        #start playing the game
        screen_obs, minimap_obs, non_spatial_obs = transform_obs(timesteps_current) #note: the obs shapes are (84,84,17), (84,84,7), (7)
        while (not done) and timer < timer_limit:
            screen_state_stack = np.vstack((screen_state_stack, screen_obs))
            minimap_state_stack = np.vstack((minimap_state_stack, minimap_obs))
            non_space_state_stack = np.vstack((non_space_state_stack, non_spatial_obs))

            feed_dict ={NN.screen_input_: screen_obs, NN.minimap_input_: minimap_obs , NN.non_spatial_input_: non_spatial_obs}
            non_spatial_policy, p_spatial_policy, s_spatial_policy , value = sess.run([NN.policy, NN.primary_action_spatial_,\
             NN.secondary_action_spatial_, NN.value], feed_dict=feed_dict  )
            # print("spatial policy sum: ", np.sum(p_spatial_policy))
            # print("spatial policy : ", (p_spatial_policy))
            [action_step, non_spatial_determine , primary_spatial_determine , secondary_spatial_determine, is_tuple ] = action(non_spatial_policy, p_spatial_policy, s_spatial_policy, timesteps_current)
            ...
            # print("secondary_spatial_determine shape: ", secondary_spatial_determine.shape)
            ...
            next_screen_state_stack = np.vstack((next_screen_state_stack, screen_obs))
            next_minimap_state_stack = np.vstack((next_minimap_state_stack, minimap_obs))
            next_non_space_state_stack = np.vstack((next_non_space_state_stack, non_spatial_obs))
            ...
            action_spatial_secondary_stack = np.vstack((action_spatial_secondary_stack, secondary_spatial_determine.reshape(1 , *secondary_spatial_determine.shape )))
            reward_stack = np.vstack((reward_stack, reward.reshape(1 , 1 )))
            value_stack = np.vstack((value_stack, value.reshape(1,1)))
            is_primary_stack = np.vstack((is_primary_stack,is_tuple[0]))
            is_secondary_stack = np.vstack((is_secondary_stack,is_tuple[1]))
...
screen_size, minimap_size, non_spatial_size, non_spatial_action_size,spatial_action_size, total_episodes): 
    ...
    secondary_spatial_action_batch = np.empty((0,*spatial_action_size),float)
    reward_batch =   np.empty((0,1),float)
    current_value_batch =  np.empty((0,1),float)
    is_primary_batch =  np.empty((0,1),float)
    is_secondary_batch =  np.empty((0,1),float)
    ...
    for iteration in range(batch_size):
        ...
        secondary_spatial_memory_clip = memory["action_spatial_secondary"][i][j:j+batch_frame_len]
        secondary_spatial_action_batch = np.vstack((secondary_spatial_action_batch, secondary_spatial_memory_clip.reshape(( *secondary_spatial_memory_clip.shape)) ))
        ...
        is_secondary_memory_clip = memory["is_secondary"][i][j:j+batch_frame_len]
        is_secondary_batch =  np.vstack((is_secondary_batch, is_secondary_memory_clip)) 
    ...
    feed_dict={NN.screen_input_: screen_batch,
                                    ...
                                    NN.is_secondary : is_secondary_batch
                                     }

                        
    sess.run( [NN.apply_grads], #what happens when theres two inputs
                        feed_dict=feed_dict)
